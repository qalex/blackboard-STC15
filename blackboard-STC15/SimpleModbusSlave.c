...
#define CYCLES_PER_US MAIN_Fosc/1000000

// frame[] is used to recieve and transmit packages. 
// The maximum serial ring buffer size is 128
unsigned char __xdata frame[BUFFER_SIZE];
unsigned int __idata holdingRegsSize; // size of the register array 
unsigned char __idata broadcastFlag;
unsigned char __idata slaveID;
unsigned char __idata function;
unsigned char __idata TxEnablePin;
unsigned int __idata errorCount;
unsigned char __idata  T1_5; // inter character time out
unsigned char __idata T3_5; // frame delay
...
unsigned int modbus_update(unsigned int *holdingRegs)
{
  ...
  while(serial_available())// while (Serial.available()) 
  {
    ...
    else
    {
      if (buffer == BUFFER_SIZE)
        overflow = 1;
      frame[buffer] = serial_getc();
      buffer++;
    }
    delay_5us(T1_5); // inter character time out
  }
...
void modbus_configure(unsigned char _slaveID, unsigned char _TxEnablePin, unsigned int _holdingRegsSize)
{
  slaveID = _slaveID;
  serial_init();
  ...
  {
    T1_5 = (15000000/baud)/5; // 1T * 1.5 = T1.5
    T3_5 = (35000000/baud)/5; // 1T * 3.5 = T3.5
  }
  */
